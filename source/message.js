...
async function LoadDataBase(conn, m) {
	try {
		...
		if (setBot) {
            if (!('afk' in setBot)) setBot.afk = false
			...
			if (!('antitagowner' in setBot)) setBot.antitagowner = false
		...
		if (m.isGroup) {
			...
			if (group) {
			    if (!('afk' in group)) group.afk = false
                if (!('antitagowner' in group)) group.antitagowner = false
...
async function MessagesUpsert(conn, message, store) {
	try {
		...
		require('../sikima.js')(conn, m, message, store);
